...
use crate::network;
use crate::crypto::{decrypt_iv,decrypt_message,derive_aes_key_from_keycode,encrypt_iv,encrypt_message};
...
pub fn send_command(host: &str, port: u16, keycode: &str, salt: &[u8; 16], mut command: String) -> Result<String, Box<dyn Error>> {
    ...
    encrypted_message.extend_from_slice(&encrypt_message(command.as_str(), &randomly_generated_iv, &aes_key));

    let response = match network::send_and_receive_tcp_message((host, port), &encrypted_message) {
        Ok(response_bytes) => response_bytes,
        Err(e) if e.kind() == io::ErrorKind::WouldBlock => return Err(Box::new(CommunicationError::InvalidCommand(command))),
    ...
    let decrypted_response_message = decrypt_message(encrypted_response_message, &decrypted_response_iv, &aes_key);

    let null_char_position = decrypted_response_message.iter().position(|b| *b == 0).unwrap_or(decrypted_response_message.len());
